...
Module: mcp_server
This module sets up an MCP server with tooling for searching the web and generating
documentation for a codebase. It also includes a testing entry point (UNIT_test)
that can be invoked to validate basic behavior via unit tests with mocks.
"""
...
def UNIT_test():
    """_______GENERATED BY CODE_AGENT_______
    Unit tests for the mcp_server module. Uses unittest framework and mocks
    to validate behavior without requiring real external dependencies.
    Returns a unittest result object.
    """
    import unittest
    from unittest.mock import patch

    # Import the module itself to patch and test internal components
    mod = __import__(__name__, fromlist=[''])

    class DummyFileBase1:
        updated_map = {}
        def __init__(self, directory):
            self.directory = directory
            self.files = ['sample.py']
            self._idx = 0
        def traverse(self):
            return 1
        def get_file(self):
            if self._idx < len(self.files):
                f = self.files[self._idx]
                self._idx += 1
                return f
            return None
        def file_content(self, f):
            return "def foo():\n    return 1"
        def update_file_content(self, f, content):
            DummyFileBase1.updated_map[f] = content

    class DummyFileBase2:
        updated_map = {}
        def __init__(self, directory):
            self.directory = directory
            self.files = ['not_python.txt']
            self._idx = 0
        def traverse(self):
            return 1
        def get_file(self):
        ...
        def file_content(self, f):
            return "content"
        def update_file_content(self, f, content):
            DummyFileBase2.updated_map[f] = content

    class TestMCPServer(unittest.TestCase):
        def test_document_codebase_process_python_file(self):
            with patch.object(mod, 'FileBase', DummyFileBase1):
                with patch.object(mod, 'stream_graph_updates', lambda payload: "\nprint('ok')\n"):
                    res = mod.document_codebase(dir="dummy_dir")
                    self.assertEqual(res.get('status'), 'success')
                    self.assertIn('sample.py', res.get('files_processed', []))
                    self.assertIn('sample.py', DummyFileBase1.updated_map)

        def test_document_codebase_skips_non_python_file(self):
            with patch.object(mod, 'FileBase', DummyFileBase2):
                with patch.object(mod, 'stream_graph_updates', lambda payload: "should not be called"):
                    res = mod.document_codebase(dir="dummy_dir")
                    self.assertEqual(res.get('status'), 'success')
                    self.assertEqual(res.get('files_processed'), [])
                    self.assertEqual(res.get('count'), 0)

    suite = unittest.TestSuite()
    suite.addTest(TestMCPServer('test_document_codebase_process_python_file'))
    suite.addTest(TestMCPServer('test_document_codebase_skips_non_python_file'))
