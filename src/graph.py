...
def UNIT_test():
    """_______GENERATED BY CODE_AGENT_______
    Lightweight unit tests for the graph code. Uses a minimal unittest-based runner.
    """
    import unittest
    class _Tests(unittest.TestCase):
        def test_code_agent_returns_expected(self):
            original_invoke = llm.invoke
            class DummyMessage:
                def __init__(self, content): self.content = content
            def dummy_invoke(_state_messages): return DummyMessage("dummy-response")
            llm.invoke = dummy_invoke
            try:
                res = code_agent({"messages": []})
                self.assertIn("messages", res)
                self.assertIsInstance(res["messages"], list)
                self.assertEqual(len(res["messages"]), 1)
                self.assertTrue(hasattr(res["messages"][0], "content"))
                self.assertEqual(res["messages"][0].content, "dummy-response")
            finally:
                llm.invoke = original_invoke

        def test_stream_graph_updates_uses_dummy(self):
            original_invoke = llm.invoke
            class DummyMessage:
                def __init__(self, content): self.content = content
            def dummy_invoke(_state_messages): return DummyMessage("dummy-response")
            llm.invoke = dummy_invoke
            try:
                content = stream_graph_updates("any")
                self.assertEqual(content, "dummy-response")
            finally:
                llm.invoke = original_invoke
