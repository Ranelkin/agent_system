...
def UNIT_test():
    """_______GENERATED BY CODE_AGENT_______
    Unit tests for MCPTool focused on the call_tool method behavior without requiring
    a live MCP server. Uses lightweight fakes for the session object.
    """
    import unittest
    import asyncio
    ...
    class _FakeSessionString:
        async def call_tool(self, tool_name, arguments):
            return "OK"

    class TestMCPTool(unittest.TestCase):
        def setUp(self):
            self.tool = MCPTool("dummy")

        def test_call_tool_with_dict_result(self):
            self.tool.session = _FakeSessionDict()
            self.tool.loop = asyncio.new_event_loop()
            result = self.tool.call_tool("mytool", {"a": 1})
            self.assertTrue(result.startswith('{\n  "tool": "mytool"'))

        def test_call_tool_with_string_result(self):
            self.tool.session = _FakeSessionString()
            self.tool.loop = asyncio.new_event_loop()
            result = self.tool.call_tool("mytool2", {"x": 2})
            self.assertEqual(result, "OK")

    suite = unittest.defaultTestLoader.loadTestsFromTestCase(TestMCPTool)
